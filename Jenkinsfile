...
pipeline {
        ...
        stage('Jmeter') {
          steps {
            script{
                ...
                sh "docker run --network backend --interactive --rm --volume `pwd`/jmeter:/jmeter egaillardon/jmeter --nongui --testfile boat-house.jmx --logfile output/result.jtl -e -o ./output"
                sh "ls -al ./jmeter"
    ...
    post {
      always {
        sh "sudo rm -rf product-service/api/target || exit 0"
      }
    }
